...
function SpotifyWidget({ track }: { track: SpotifyTrack | null }) {
  const title = track?.title && !/^spotify(?:\s+premium)?$/i.test(track.title) ? track.title : "Spotify";
  const artist =
    track?.artist && track.artist !== "Spotify"
      ? track.artist
      : track?.title && !/^spotify(?:\s+premium)?$/i.test(track.title)
        ? ""
  ...
  return (
    <div className="flex w-[280px] items-center gap-2.5 rounded-xl bg-black/85 px-2 py-1.5">
      {track?.artwork ? (
        <img src={track.artwork} alt="" className="h-11 w-11 shrink-0 rounded-md object-cover" />
      ) : (
        <div className="flex h-11 w-11 shrink-0 items-center justify-center rounded-md bg-[#1db954] text-black">
      ...
      <div className="min-w-0 flex-1">
        <div className="truncate text-[13px] font-semibold leading-tight text-white">{title}</div>
        <div className="truncate text-[11px] leading-tight text-zinc-400">{artist}</div>
