...
export function NoticesPage() {
  ...
  const remove = async (id: number) => {
    if (!editor || id <= 0 || !window.synvity?.noticesDelete) return;
    setBusyId(id);
    setMessage("");
    const previous = notices;
    setNotices((rows) => rows.filter((row) => row.id !== id));
    try {
      const result = await window.synvity.noticesDelete(id);
      if (result?.notices) setNotices(result.notices);
      if (!result?.ok) {
        setNotices(previous);
          ...
          <div className="ink-card p-5">
            ...
            <label className="mt-4 block text-[12px] text-zinc-500">
              ...
              <input
                ...
                maxLength={191}
                placeholder={kind === "announcement" ? "Promuj Aries panel" : "1.0.99"}
