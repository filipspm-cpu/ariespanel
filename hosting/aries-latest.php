...
function aries_from_latest_yml() {
  $raw = aries_http_get("https://github.com/filipspm-cpu/ariespanel/releases/latest/download/latest.yml");
  if ($raw === "" || !preg_match("/^version:\\s*([0-9][0-9A-Za-z.\\-]+)/m", $raw, $vm)) return null;
  $version = $vm[1];
  $name = "ARIES-Setup-" . $version . ".exe";
  if (preg_match("/^path:\\s*(ARIES-Setup-.+\\.exe)/m", $raw, $pm)) $name = $pm[1];
  $url = "https://github.com/filipspm-cpu/ariespanel/releases/latest/download/" . rawurlencode($name);
  return array(
    "tag" => "v" . $version,
    "version" => $version,
    "name" => $name,
...
function aries_latest_release() {
  ...
  if (is_file($cache) && (time() - filemtime($cache)) < 600) {
    ...
    if (is_array($cached) && !empty($cached["url"]) && !empty($cached["name"]) && aries_github_ok($cached["url"])) {
      return $cached;
    }
  }
  $out = aries_from_api();
  if (!$out) $out = aries_from_latest_yml();
  if (!$out || !aries_github_ok($out["url"])) return null;
